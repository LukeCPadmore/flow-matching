...
from flow_matching.utils.train import create_pil_image, unpack_batch
...
class ColouriserSamplingSweepCallback(Callback):
    def __init__(
        self,
        ode_solver_names: list[str] | None = None,
        n_steps_list: list[int] | None = None,
        guidance_scales: list[float] | None = None,
        n_eval_images: int = 64,
        n_grid_images: int = 8,
        clamp_mode: str = "clamp",
        sample_seed: int | None = 22,
        artifact_prefix: str = "colouriser/sweep",
    ) -> None:
        self.ode_solver_names = ode_solver_names or ["euler_solver", "rk4_solver"]
        self.n_steps_list = [int(n) for n in (n_steps_list or [10, 20, 50])]
        self.guidance_scales = [
            float(scale) for scale in (guidance_scales or [1.0, 1.5, 2.0])
        ]
        self.n_eval_images = int(n_eval_images)
        self.n_grid_images = int(n_grid_images)
        self.clamp_mode = clamp_mode
        self.sample_seed = sample_seed
        self.artifact_prefix = artifact_prefix
        self._lpips_metric: LearnedPerceptualImagePatchSimilarity | None = None
    ...
    def on_validation_start(
        ...
        if not trainer.is_global_zero:
            return
        if trainer.datamodule is None:
            raise RuntimeError("ColouriserSamplingSweepCallback requires a datamodule.")
        ...
        try:
            ...
            raise RuntimeError(
                "ColouriserSamplingSweepCallback requires LPIPS vgg weights to be available in the environment."
            ) from exc
    ...
    def on_validation_epoch_end(
        ...
        if self._lpips_metric is None:
            raise RuntimeError(
                "ColouriserSamplingSweepCallback was not initialised correctly."
            )
        ...
        LAB = self._collect_lab_subset(datamodule.val_dataloader(), pl_module.device)
        L = LAB[:, :1, ...]
        ab = LAB[:, 1:, ...]
        gt_rgb = lab_to_rgb(LAB)
        ...
        was_training = pl_module.training
        pl_module.eval()
        summary_tiles: list[torch.Tensor] = []

        with torch.no_grad():
            for ode_solver_name in self.ode_solver_names:
                ode_solver = get_ode_solver_from_name(ode_solver_name)
                for n_steps in self.n_steps_list:
                    for guidance_scale in self.guidance_scales:
                        suffix = _sweep_metric_suffix(
                            ode_solver_name, n_steps, guidance_scale
                        )
                        seconds, ab_pred = self._timed_sample(
                            pl_module,
                            L,
                            ode_solver=ode_solver,
                            n_steps=n_steps,
                        ...
                        pred_lab = torch.cat([L, ab_pred.clamp(-1.0, 1.0)], dim=1)
                        pred_rgb = lab_to_rgb(pred_lab)
                        lpips = self._compute_lpips(pred_rgb, gt_rgb, pl_module.device)
                        rgb_mse = F.mse_loss(pred_rgb, gt_rgb)
                        ab_mse = F.mse_loss(ab_pred, ab)
                        images_per_second = LAB.shape[0] / max(seconds, 1e-12)

                        self._log_sweep_metrics(
                            pl_module,
                            suffix,
                            lpips=lpips,
                            rgb_mse=rgb_mse,
                        ...
                        summary_tiles.append(
                            torch.cat(
                                [L[:1].repeat(1, 3, 1, 1), pred_rgb[:1], gt_rgb[:1]],
                                dim=-1,
                            )
                        )
        ...
        if summary_tiles:
            summary_image = create_pil_image(torch.cat(summary_tiles, dim=0), nrow=1)
            _log_sample_image(trainer, summary_image, f"{self.artifact_prefix}/summary")

    def _collect_lab_subset(self, loader, device: torch.device) -> torch.Tensor:
        batches: list[torch.Tensor] = []
        sample_count = 0
        for batch in loader:
            LAB, _ = unpack_batch(batch)
            take = min(LAB.shape[0], self.n_eval_images - sample_count)
            if take <= 0:
                break
            batches.append(LAB[:take])
            sample_count += take

        if sample_count <= 0:
            raise RuntimeError("ColouriserSamplingSweepCallback found no validation samples.")
    ...
    def _timed_sample(
        ...
        *,
        ...
        if pl_module.device.type == "cuda":
            torch.cuda.synchronize(pl_module.device)
        return time.perf_counter() - start, ab_pred
    ...
    def _compute_lpips(
        ...
        self._lpips_metric = self._lpips_metric.to(device)
        self._lpips_metric.reset()
        self._lpips_metric.update(
            pred_rgb.mul(2.0).sub(1.0),
            gt_rgb.mul(2.0).sub(1.0),
        )
        lpips = self._lpips_metric.compute()
        self._lpips_metric.reset()
        return lpips

    def _log_sweep_metrics(
        self,
        pl_module: pl.LightningModule,
        suffix: str,
        *,
        ...
        metrics = {
            f"val_sweep_lpips_{suffix}": lpips,
            f"val_sweep_rgb_mse_{suffix}": rgb_mse,
            f"val_sweep_ab_mse_{suffix}": ab_mse,
            f"val_sweep_seconds_{suffix}": torch.tensor(
                seconds, device=pl_module.device, dtype=torch.float32
            ),
            f"val_sweep_images_per_second_{suffix}": torch.tensor(
                images_per_second, device=pl_module.device, dtype=torch.float32
            ),
        }
    ...
    def _log_setting_grid(
        ...
        n = min(self.n_grid_images, L.shape[0])
        if n <= 0:
            return
        comparison_tiles = torch.cat(
            [L[:n].repeat(1, 3, 1, 1), pred_rgb[:n], gt_rgb[:n]],
            dim=-1,
        )
        image = create_pil_image(comparison_tiles, nrow=1)
        _log_sample_image(trainer, image, f"{self.artifact_prefix}/{suffix}")


def _sweep_metric_suffix(
    ode_solver_name: str,
    n_steps: int,
    guidance_scale: float,
) -> str:
    cfg = f"{guidance_scale:g}".replace("-", "m").replace(".", "_")
    return f"{ode_solver_name}_steps_{n_steps}_cfg_{cfg}"
